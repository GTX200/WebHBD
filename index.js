...
function safeFilename(value, format) {
  const clean = String(value || "GHZ-Media")
    .replace(/[\\/:*?"<>|\x00-\x1F]/g, " ")
    .replace(/\s+/g, " ")
    .trim()
  ...
  return clean.toLowerCase().endsWith("." + ext)
    ? clean
    : clean + "." + ext;
}
...
function findMediaUrl(data, format) {
  ...
  const score = item => {
    ...
    if (format === "mp4") {
      if (/\.(mp4|webm)(?:[?#]|$)/.test(lowerUrl)) points += 40;
      if (/\.(jpg|jpeg|png|webp)(?:[?#]|$)/.test(lowerUrl)) points -= 120;
      if (/thumbnail|avatar|profile|cover/.test(lowerUrl)) points -= 100;
    }

    if (format === "mp3") {
      if (/\.(mp3|m4a|aac|opus|ogg)(?:[?#]|$)/.test(lowerUrl)) points += 40;
      if (/\.(jpg|jpeg|png|webp|mp4|webm)(?:[?#]|$)/.test(lowerUrl)) points -= 100;
    }

    if (format === "jpg") {
      if (/\.(jpg|jpeg|png|webp)(?:[?#]|$)/.test(lowerUrl)) points += 40;
    }
    ...
    if (
      ...
      try {
        ...
        if (!upstream.ok && upstream.status !== 206) {
          return json({
            success:false,
            error:"Server media menolak permintaan (HTTP " + upstream.status + ")."
          }, 502);
        }
        ...
        return json({
          success:false,
          error:"Gagal mengambil file media dari server sumber."
        }, 502);
      }
    }
